...
Addresses.prototype.summary = function(addresses, callback) {
  ...
  var makeRequest = function(addr, callback) {
    request.get(self.url + addr).end(function(res) {
      ...
      var data = {
        address: addr,
        balance: res.body.balanceSat + res.body.unconfirmedBalanceSat, //this could be a problem for altcoins that don't fit in a JS number (53 bits)
...
Addresses.prototype.transactions = function(addresses, height, callback) {
  var self = this
  var addrsStr = Array.isArray(addresses) ? addresses.join(',') : addresses

  if (typeof height === 'number') {
    console.warn('block height is not supported')
  } else if (typeof height === 'function') {
    callback = height
  }

  // addrs/addr1,addr2,.../txs
  var base = self.url.replace('addr/', 'addrs/')
  // from/to is a hack to avoid paging through results
  // and get them all at once
  request.get(base + addrsStr + '/txs?from=0&to=100000').end(function(res) {
    if (!res.ok) return callback(new Error('non-ok http status code'), res)

    var results = res.body.items
    var txInfos = results
      .map(utils.txFromJSON, utils)
      .map(function (tx, i) {
        return {
          txId: tx.getId(),
          txHex: tx.toHex(),
          blockId: utils.reverseHexBuffer(results[i].blockhash).toString('hex'),
          confirmations: results[i].confirmations
        }
      })

    callback(null, txInfos)
  })
}
